# Daniel Redwine
# negative binomial instrinsic analyses
...
aggression_nb_model <- glmer.nb(Total_Agonistic~PCRsex*PCRMorph*Winter+(1|SampleID) +
...
recipient_nb_model <- glmer.nb(Total_Recipient~PCRsex*PCRMorph*Winter + (1|SampleID) 
                               + offset(log(Platform_Time)), data = total_data)
...
sw(dredge_recipient_nb) #notice this is the global model, not just the competitive model set
